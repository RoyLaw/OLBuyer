...
uses
  ...
  System.Classes, Vcl.Stdctrls, StrUtils, Dialogs,
...
function TWebAccess.PlaceOrder(curUser: TBuyer; curItem: TStoreItem): string;
// 临时
var
  Params: TStrings;
  i: integer;
...
begin
  ...
  FIdHTTP.Request.AcceptLanguage := 'en-us,zh-cn;q=0.7,en;q=0.3';
  // FIdHTTP.Request.AcceptEncoding := 'gzip, deflate';
  FIdHTTP.Request.Connection := 'keep-alive';

  Params := TStringList.Create;
  Params.Add('login=' + curUser.StoreAccount);
  Params.Add('rememberMe=false');
  Params.Add('password=' + curUser.StorePassword);

  try
    FIdHTTP.Post
      ('https://www.nike.com/profile/login?Content-Locale=zh_CN', Params);
  except
    on E: Exception do
    begin
      ShowMessage('ERR: Can''t Login.');
      Result := 'Not Logged In.' + E.Message;
    end;
  end;
  ...
  begin
    ...
    bufLinkAmount := ExtractHtmlTagValues(bufHTML, 'option', 'value', bufLinks);
    bufResult := '';
  end;
  if bufLinkAmount > 0 then
  begin
